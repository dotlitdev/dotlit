...
const version = "sw:0.0.15 dotlit: " + typeof dotlit
let root = ''
...
const getMockResponse = async (event) => {
    ...
    return new Response(`version: ${version} root: ${root} url: ${event.request.url}`)
...
self.addEventListener('install', event => {
  root = new URL(location).searchParams.get('root')
  event.waitUntil(
